...
class wan_inv():
    ...
    def __init__(self, dim, noise_level, dm_size, bd_size, beta_u, beta_bd,
        ...
        self.up, self.low=   1.0, -1.0           #the domain [-1,1]^d
        self.c_u= [0.0, 0.0]+[0.0]*(dim-2)       #the global minimum point of u(x)
        self.k_u= [1.0, 1.0]+[1.0]*(dim-2)
        #
        self.c_1= [-0.5, 0.5]+[0.0]*(dim-2)      #the coefficient a_1(x)  
        self.k_1= [0.25, 1.0]+[0.01]*(dim-2)
        self.alpha_1= 0.2
        self.gamma_1= 2.0                               
        self.c_2= [0.5, -0.5]+[0.0]*(dim-2)       #the coefficient a_2(x)
        self.k_2= [1.0, 0.36]+[0.01]*(dim-2)
        self.alpha_2= 0.2
        self.gamma_2= 2.0
        #
    ...
    def sample_train(self, dm_size, bd_size, dim):
        ...
        #*********************************************************
        # The value of f(x)
        a1, a2, omega_a1, omega_a2= 0.0, 0.0, 0.0, 0.0
        for i in range(len(self.k_1)):
            a1= a1+self.k_1[i]*(x_dm[:,i]-self.c_1[i])**2
            a2= a2+self.k_2[i]*(x_dm[:,i]-self.c_2[i])**2
            omega_a1= omega_a1+4*self.k_u[i]*self.k_1[i]*(x_dm[:,i]-self.c_1[i])*x_dm[:,i]/self.alpha_1
            omega_a2= omega_a2+4*self.k_u[i]*self.k_2[i]*(x_dm[:,i]-self.c_2[i])*x_dm[:,i]/self.alpha_2
        a1= self.gamma_1*np.exp(-a1/self.alpha_1)
        a2= self.gamma_2*np.exp(-a2/self.alpha_2)
        #
        omega_u, k_u1, k_u2= 0.0, 0.0, 0.0
        for i in range(dim):
            omega_u= omega_u+self.k_u[i]*(x_dm[:,i]-self.c_u[i])**2
            k_u1= k_u1-2*self.k_u[i]
            k_u2= k_u2-4*self.k_u[i]**2*x_dm[:,i]**2
        #
        part_one= omega_a1*np.sin(omega_u)*a1+omega_a2*np.sin(omega_u)*a2
        part_two= (a1+a2)*(k_u1*np.sin(omega_u)+k_u2*np.cos(omega_u))
        #
        ...
        u_bd= 0.0
        for i in range(dim):
            u_bd= u_bd+self.k_u[i]*(x_bd[:,i]-self.c_u[i])**2
        u_bd= np.cos(u_bd)
        ...
        a1, a2= 0.0, 0.0
        for i in range(len(self.k_1)):
            a1= a1+self.k_1[i]*(x_bd[:,i]-self.c_1[i])**2
            a2= a2+self.k_2[i]*(x_bd[:,i]-self.c_2[i])**2
        a1= self.gamma_1*np.exp(-a1/self.alpha_1)
        a2= self.gamma_2*np.exp(-a2/self.alpha_2)
        #
        a_bd= a1+a2
    ...
    def sample_test(self, mesh_size, dim):
        ...
        u_draw_dm= 0.0
        for i in range(dim):
            u_dm= u_dm+self.k_u[i]*(x_dm[:,i]-self.c_u[i])**2
            u_draw_dm= u_draw_dm+self.k_u[i]*(x_draw_dm[:,i]-self.c_u[i])**2
        u_dm= np.cos(u_dm)
        u_draw_dm= np.cos(u_draw_dm)
        u_dm= np.reshape(u_dm, [-1, 1])
        u_draw_dm= np.reshape(u_draw_dm, [-1, 1])
        #***********************************************************
        # The exact a(x)
        a1, a2= 0.0, 0.0
        a1_draw, a2_draw= 0.0, 0.0
        for i in range(len(self.k_1)):
            a1= a1+self.k_1[i]*(x_dm[:,i]-self.c_1[i])**2
            a2= a2+self.k_2[i]*(x_dm[:,i]-self.c_2[i])**2
            a1_draw= a1_draw+self.k_1[i]*(x_draw_dm[:,i]-self.c_1[i])**2
            a2_draw= a2_draw+self.k_2[i]*(x_draw_dm[:,i]-self.c_2[i])**2
        a1= self.gamma_1*np.exp(-a1/self.alpha_1)
        a1_draw= self.gamma_1*np.exp(-a1_draw/self.alpha_1)
        a2= self.gamma_2*np.exp(-a2/self.alpha_2)
        a2_draw= self.gamma_2*np.exp(-a2_draw/self.alpha_2)
        a_dm= a1+a2
        a_draw_dm= a1_draw+a2_draw
    ...
    def net_a(self, x_in, out_size, name, reuse):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            hi= tf.sin(tf.layers.dense(hi, h_size), name='output_layer1')
    ...
    def fun_g(self, x, n_vec):
        ...
        for i in range(self.dim):
            u_val= tf.add(u_val, tf.multiply(self.k_u[i], tf.pow(x_list[i]-self.c_u[i], 2)))
        u_val= tf.cos(u_val)
    ...
    def build(self):
        ...
        with tf.variable_scope('loss'):
            with tf.name_scope('loss_u'):
                ...
                #
                #intw_r1= tf.reduce_mean(tf.multiply(self.a_val, dudw_val))
                #intw_l1= tf.reduce_mean(tf.multiply(self.f_val, w_val))
                #self.loss_intw= 10.0*tf.square(intw_l1-intw_r1) 
                #*******************************************************************
...
if __name__=='__main__':
    ...
    file_path='./iwan_smooth/'  # The filepath for saving data (and figures)
