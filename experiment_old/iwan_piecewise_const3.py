...
class wan_inv():
    ...
    def __init__(self, file_name, dim, beta_u, beta_a, N_dm, N_bd):
        ...
        self.k1=   [1.0, 0.8]+[0.01]*(dim-2)     #\omega_1区域表达式中:前面的系数
        self.k2=   [2.0, 1.0]+[0.01]*(dim-2)
        self.c_a1= [0.1, 0.1]+[0.0]*(dim-2)       #\omega_1区域表达式中:区域的中心
        self.c_a2= [0.05, 0.05]+[0.0]*(dim-2)
        self.c_u= [0, 0]+[0.0]*(dim-2)           #真实解表达式中:最小值点
        self.r1= 0.6                              #\omega_1区域表达式中：半径值
        self.r2= 0.4
    ...
    def build(self):
        ...
        with tf.variable_scope('loss'):
            with tf.name_scope('loss_u'):
                ...
                self.loss_int= 10.0*tf.square(int_l1-int_r1) / test_norm_u
                #*******************************************************************
                ...
                self.loss_int_a= 10.0*tf.square(int_l1_a-int_r1_a) / test_norm_a    
                #
        ...
        with tf.Session() as sess:
            ...
            for i in range(self.iteration):
                ...
                if i%500==0:
                    ...
                    #visual.show_v_val(mesh, draw_x, pred_v_draw, 'v',  i)
                #
